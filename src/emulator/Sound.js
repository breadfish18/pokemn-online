(function() {
  ...
  VBASound.prototype.getNumExtraSamples = function() {
    ...
    return samples >= 0
      ? samples
      : samples + this.audioSpareSamplesRingBuffer.length;
  };
  ...
  VBASound.prototype.writeSound = function(pointer8, length16) {
    ...
    for (i = 0; i < length16; i++) {
      this.audioSpareSamplesRingBuffer[this.audioSpareWritePtr] =
        heap16[pointer16 + i];
    ...
    for (i = 0; i < requiredSamples; i++) {
      for (channel = 0; channel < numChannels; channel++) {
        if (this.audioSpareReadPtr === this.audioSpareWritePtr) {
          ...
          audioBuffers[channel][i] =
            this.audioSpareSamplesRingBuffer[this.audioSpareReadPtr] / 0x4000;
